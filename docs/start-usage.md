...
`file`은 현재 프로젝트 기준의 기존 파일이다. 작업표 자체, `.oml`, `.git`, 비밀 파일, 상위 경로는 수정할 수 없다. 작업표·검증기 보호는 경로 이름뿐 아니라 실제 파일 동일성도 확인해 대소문자 별칭을 차단한다. 한 번의 수정은 UTF-8 파일 하나·64 KiB 이하이며, 자연어 목표만 입력해서 파일이나 작업 계획을 자동 생성하는 기능은 아니다.
