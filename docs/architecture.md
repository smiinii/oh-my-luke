...
완료 조건은 실제 명령을 실행하지 않고 종료 코드, 파일 존재, 중대 오류 개수, 요구사항 충족 여부로만 판정한다. 경로 정책은 프로젝트 밖과 외부 심볼릭 링크를 차단한다. 명령 정책은 실행 파일과 인자 접두사를 분리한 허용 목록을 사용하며 셸 실행은 허용 목록에 잘못 추가되어도 거부한다. 외부 변경과 삭제는 명시적 승인이 필요하다.
