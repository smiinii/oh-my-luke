...
완료 조건은 실제 명령을 실행하지 않고 종료 코드, 파일 존재, 중대 오류 개수, 요구사항 충족 여부로만 판정한다. 경로 정책은 원시 `..`, 프로젝트 밖 경로, 외부 심볼릭 링크와 최종 심볼릭 링크 접근을 차단한다. 명령 정책은 운영자가 신뢰한 절대 실행 파일과 전체 인자 목록이 정확히 일치하는 허용 목록을 사용하며 셸·명령 래퍼는 허용 목록에 잘못 추가되어도 거부한다. 외부 변경과 삭제 승인은 정확한 명령 또는 경로, 실행 ID, 위험 등급, 만료 시각에 결속된다.
