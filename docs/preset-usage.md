...
이 문서의 `run`은 작업표에 모드가 있는 기존 명시적 진입점이다. 새 작업표에서 자동 또는 수동을 선택하려면 [공통 시작 명령](start-usage.md)을 사용한다.
...
- 64 KiB 이하의 파일 본문·제안은 내용 주소형 아티팩트로 저장하고 상태에는 해시를 둔다. 읽을 때 해시와 링크 경계를 검사한다.
- 적용은 FileTool 잠금 안에서 기준 내용 비교 → 체크포인트 → 쓰기 순서다. 이미 적용된 동일 작업은 재사용한다. 외부 편집기와 완전한 원자적 동시 편집까지 보장하지는 않는다.
- 실패 시 최근 실패 코드·제한된 진단과 현재 파일만 다음 writer의 명시적 입력에 포함한다. Codex의 별도 프로젝트 읽기 기능까지 막는 것은 아니다.
- 프리셋 시도·동일 실패·같은 제안 횟수는 별도로 저장한다. 사용자 결과는 `PresetResult`로 구분하고 검증 성공일 때만 CLI 종료 코드 0이다.
- 시간·기록 사용량은 노드 경계에서 검사한다. 한 호출만큼 초과할 수 있으며, 제안이 나왔더라도 다음 적용·검증 전에 중단될 수 있다.
