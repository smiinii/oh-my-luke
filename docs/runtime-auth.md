...
한 프롬프트의 비재개 `exec`에서 식별 불가능한 여러 완료 이벤트·여러 스레드 시작이 보이면 중복/추가 호출을 추측해 합산하지 않는다. 합계의 정수 범위 초과도 미확인으로 처리한다. JSON 중복 키·후행 값은 유효한 응답으로 받지 않는다. 프로젝트별 조회와 저장 결과 대조는 [질문별 사용량](usage-records.md)을 따른다.
